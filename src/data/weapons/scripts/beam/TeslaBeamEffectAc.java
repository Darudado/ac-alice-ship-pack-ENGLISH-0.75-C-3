...
public class TeslaBeamEffectAc implements BeamEffectPlugin{
    ...
    public void advance(float amount, CombatEngineAPI engine, BeamAPI beam) {
        ...
        if (target instanceof ShipAPI && beam.getBrightness() >= 1f) {
            ...
            if (fireInterval.intervalElapsed()) {
                ...
                float pierceChance = Min( 2*((ShipAPI)target).getHardFluxLevel() , 1f);//在对面幅能过半时100%穿盾

                pierceChance *= ship.getMutableStats().getDynamic().getValue(Stats.SHIELD_PIERCED_MULT);

                boolean piercedShield = hitShield && (float) Math.random() < pierceChance;//穿盾判定
                //piercedShield = true;

                if (!hitShield || piercedShield) {
                    Vector2f point = beam.getRayEndPrevFrame();
                    float emp = beam.getDamage().getFluxComponent() * 0.5f;//s=伤害不高，emp极强
