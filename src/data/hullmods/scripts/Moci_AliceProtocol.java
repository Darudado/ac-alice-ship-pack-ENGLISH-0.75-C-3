 ...
 */
public class Moci_AliceProtocol extends BaseHullMod {
	...
	public static class AliceProtocolRangeModifier implements WeaponBaseRangeModifier {
		...
		public float getWeaponBaseRangeFlatMod(ShipAPI ship, WeaponAPI weapon) {
			...
			if (currentRange + bonus > MAX_RANGE_LIMIT) {
				bonus = MAX_RANGE_LIMIT - currentRange;
			}
	...
	public String getDescriptionParam(int index, HullSize hullSize) {
		...
		if (index == 1) return "" + (int) MAX_RANGE_LIMIT;
		return null;
	}
	...
	public void addPostDescriptionSection(TooltipMakerAPI tooltip, HullSize hullSize, ShipAPI ship, 
		...
		tooltip.addPara("\nThis unprecedented ship protocol not only kicks out standard Remnant drone subsystems, but also has other modifications.\n"
				+ "Base Effect: Increases weapon range through precise fire control algorithms.", opad);
		...
		tooltip.addPara("# All non-missile weapon base range limited to %s", opad, h,
				"" + (int) MAX_RANGE_LIMIT);
