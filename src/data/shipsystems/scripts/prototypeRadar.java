...
public class prototypeRadar extends BaseShipSystemScript {
    ...
    protected java.util.List<LidarArrayStats.LidarDishData> dishData = new ArrayList<LidarArrayStats.LidarDishData>();
    ...
    public void init(ShipAPI ship) {
        ...
        for (WeaponAPI w : lidar) {
            if (w.isDecorative() && w.getSpec().hasTag(Tags.LIDAR)) {
                ...
                LidarArrayStats.LidarDishData data = new LidarArrayStats.LidarDishData();
    ...
    public void rotateLidarDishes(boolean active, float effectLevel) {
        ...
        //turnRateMult = 0.1f;
        //boolean first = true;
        for (LidarArrayStats.LidarDishData data : dishData) {
...
//		stats.getEnergyWeaponRangeBonus().unmodifyPercent(id);
    ...
    public ShipSystemStatsScript.StatusData getStatusData(int index, ShipSystemStatsScript.State state, float effectLevel) {
        if (state == ShipSystemStatsScript.State.IDLE || state == ShipSystemStatsScript.State.COOLDOWN) {
            if (index == 3) {
                return new ShipSystemStatsScript.StatusData("weapon range +" + (int) PASSIVE_RANGE_BONUS + "%", false);
            }
        }
        ...
        if (index == 3) {
            return new ShipSystemStatsScript.StatusData("weapon range +" + (int) RANGE_BONUS + "%", false);
        }
        if (index == 2) {
            return new ShipSystemStatsScript.StatusData("rate of fire +" + (int) bonusPercent + "%", false);
        }
//		if (index == 1) {
//			return new StatusData("ballistic flux use -" + (int) FLUX_REDUCTION + "%", false);
//		}
        if (index == 1) {
            return new ShipSystemStatsScript.StatusData("weapon recoil -" + (int) RECOIL_BONUS + "%", false);
        }
        if (index == 0 && PROJECTILE_SPEED_BONUS > 0) {
            return new ShipSystemStatsScript.StatusData("projectile speed +" + (int) PROJECTILE_SPEED_BONUS + "%", false);
        }
        return null;
    }
